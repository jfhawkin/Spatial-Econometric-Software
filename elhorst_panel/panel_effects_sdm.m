function panel_effects_sdm(y,x,T,results,vnames,W)
...
% LeSage and Pace code for calcualting effects in a SDM model
...
if posdef ~= 0 % even cheating did not work, so punt with a kludge
    tmp = [x repmat(W,T)*y]'*[x repmat(W,T)*y];
    sigmatt = results.sige*(inv(tmp));
end;
